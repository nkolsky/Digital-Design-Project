...
always_ff @(posedge clk or negedge rst_n) begin : tickCounter
    if (!rst_n) begin
        tick_q <= 4'b0;
    end else if (clr_tick_cntr) begin
        tick_q <= 4'b0;
    end else if (run_tick_cntr && tick) begin
        tick_q <= tick_q + 1;
    end else begin
...
always_ff @(posedge clk or negedge rst_n) begin : rx_outputLogic
    if(!rst_n) begin //reset button asserted
        ...
        unique case(cur_state)
            ...
            VALIDATE_STOP: begin
                // Start counting ticks to validate stop bit at the right time
                run_tick_cntr <= 1'b1;
                //see stop_window logic above for how we use the shift reg to validate the stop bit
            end
            UPDATE_BYTE_CNT: begin
                byte_cnt_en <= 1'b1;
                //latch the byte we just received into the correct position in the message register
                msg_reg_en <= 1'b1;
